...
Complexity is seductive because each individual addition feels justified. A router here, a sub-agent there, a tool to wrap the sub-agent, a workflow to orchestrate the whole thing. Every step is locally sensible and the sum is a system nobody can debug at 2 a.m. The discipline that separates agents that ship from agents that sprawl isn't cleverness. It's restraint.
...
When an agent underperforms, the instinct is to add structure. But "add structure" is not a diagnosis. The right move depends entirely on *what kind* of gap you're looking at, and the layers are not interchangeable. A prompt fixes a different problem than knowledge does; knowledge fixes a different problem than a tool does; a tool fixes a different problem than a second agent does. Pick the wrong layer and you've added cost without closing the gap — and now you have two problems.
...
**Latency and cost.** More calls, more tokens, more models in the mix. A multi-agent system can be several times the cost and several times the wait of a single well-prompted call, for a quality bump that's sometimes zero.
...
**Debuggability.** When a single agent gives a bad answer, you read one prompt and one trace. When an orchestrator-worker-evaluator pipeline gives a bad answer, you're bisecting a distributed system to find which hand-off corrupted the state.

**Opacity.** Frameworks and extra layers tend to hide the actual prompts and responses underneath. Most "the model is dumb" bugs are really "the prompt the model received wasn't the prompt I thought I wrote" bugs — and abstraction makes those harder to see.
...
| --- | --- |
| It *behaves* wrong — wrong tone, skips steps, ignores a rule | **Prompt** (instructions) |
| It doesn't *know* a fact — about your domain, docs, or this user | **Knowledge** (retrieval) |
| It can't reliably *perform a repeatable procedure* | **Skill** (a packaged how-to) |
| It can't *reach the world* — live data, an action, computation | **Tool** |
| One context is *juggling two jobs* and doing both worse | **Agent-as-tool** |
| You need *orchestration* — ordering, gates, parallelism, approvals | **Workflow / multi-agent** |
...
A **prompt** governs *how the agent behaves*: its role, its rules, its reasoning style, the shape of its output. **Knowledge** supplies *what the agent knows*: facts, documents, the current state of your domain.

The failure mode in both directions is treating one as the other:

- **Pasting a knowledge base into the prompt.** People dump policy documents, product catalogs, and FAQs into the system prompt because the agent "needs to know" them. Now every call drags that payload, the truly important instructions get diluted in the noise, and updating a fact means redeploying a prompt. Facts belong in retrieval, fetched on demand, where they can change without touching behavior.
...
That framing keeps multi-agent honest. It forces each sub-agent to have one crisp responsibility and a contract you can test in isolation, instead of a vague "helper" that makes the system harder to reason about. It also naturally limits the blast radius: a sub-agent that summarizes can't suddenly decide to take an action, because all it returns is a summary.
...
The top of the ladder is orchestration — and it splits in a way worth naming. If you need a model to *dynamically decide* the steps, that's still the agent world (one agent calling agent-tools as it sees fit). If instead you need the steps **fixed, ordered, gated, or parallelized** — with human approvals, audit trails, business-rule enforcement — that's a **workflow**: predefined paths, not model-directed ones.

Reach for a workflow when predictability and auditability matter more than flexibility: an approval chain, a compliance pipeline, a multi-stage ETL. Reach for an agent when you genuinely can't predict the steps in advance. Using a workflow where one good prompt would do is over-engineering; using a free-roaming agent where you needed a guaranteed, auditable sequence is under-engineering. Most teams err toward the first.

## So when *do* you add complexity?

When you have evidence — not a hunch — that the simpler rung has failed.

Concretely, that means you've already done the unglamorous work: you have an evaluation setup that measures the things that matter (did the agent understand the request? did it pick the right tools with the right arguments? did the final answer stay faithful to the task?), and you can see that no amount of prompt-and-knowledge refinement moves the number. *That's* the signal to descend a rung. Not "this would be cool." Not "what if it scales." A measured ceiling.

This is the same discipline at every level. Prompt not enough? Prove it, then add knowledge. Single agent plateaued? Prove it, then split off an agent-tool. The evaluation harness is what converts "I feel like we need more" into "the data says this specific gap needs this specific layer." Without it, you're just decorating.
...
And underneath all of it, the one rule: *the best system isn't the most sophisticated one, it's the right one.* Start with one agent and a sharp prompt. Make it observable. Measure it. Then — and only then — earn the next layer.
