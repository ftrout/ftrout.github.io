...
**Wrong.** Duplicate records, mislabeled fields, a units mismatch, a scraped value that was never accurate. The model has no way to know a number is false; it treats everything in its context — all the text handed to it for a given request — as equally true and reasons confidently from a lie.
...
A bigger model does not detect any of these. It can't. Stale data isn't a reasoning problem, and a model has no independent ground truth — no separate, trusted picture of reality — to check your records against. It does what it's built to do: produce the most plausible continuation of whatever you gave it. Give it something rotten and it will produce a beautifully plausible rot.
...
The model is, by design, an engine for reasoning over whatever context it's given. Its job is to take what's in front of it and extend it coherently. It has no privileged channel to reality, no way to know that your `customer_status` field — a single column in your database meant to track where a customer stands — has meant three different things since 2023, no way to know the document it's citing (pointing to as the source of its answer) was superseded last week. When the data and the truth diverge, the model follows the data — fluently, every time. Intelligence applied to wrong inputs produces wrong outputs with more conviction, not less.
...
## Where it actually hurts: retrieval systems and agents

This stops being abstract the moment you build a retrieval system (one that looks up your documents and feeds them to the model) or an agent (an AI that doesn't just answer but takes actions in steps).

A RAG pipeline — retrieval-augmented generation, the standard pattern of "look up the relevant facts and paste them into the prompt before the model answers" — is a machine for putting your data in front of a model. If the knowledge base (the collection of documents you search over) is full of outdated docs, duplicate pages, and three versions of the same policy, retrieval will faithfully surface the wrong one, and the model will faithfully ground its answer in it — that is, base the answer on that retrieved text — and attach a citation, which makes it *look* more trustworthy, not less. You've built a system that launders bad data through a credible-looking process. The citation points at the rot.
...
- **A defined source of truth.** A source of truth is the one place you've agreed is authoritative when records disagree. When two systems disagree, something has to win. Decide what's authoritative before the model has to, not after it guessed.
- **Provenance.** Provenance just means knowing where each piece of data came from. Can you distinguish an authoritative internal record from a scraped forum post? If everything in the context is treated as equally true, the forum post wins half the time.
- **Coverage you've mapped.** Know what the system *isn't* seeing. The dangerous blind spots are the invisible ones.
- **Honest behavior when data is thin.** The system should hold back and hand off to a human when it's under-informed, not bluff. A model that says "I don't have enough to answer this" is worth more than one that always answers.
