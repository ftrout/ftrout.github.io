...
Because a decision is not a magic act. It's a conclusion drawn from context — and "context" here just means the bundle of information the system is looking at when it decides. If you don't know what context the system is using, you haven't bought a decision-maker — you've bought a confident stranger who won't tell you what they read before answering.

## A decision is downstream of its context

Strip the mystique off any "autonomous" system and you find the same machinery we've talked about before: a **model** — the underlying AI, the large language model doing the reasoning — working through a problem in a loop, and that reasoning is only ever as good as the information it's reasoning *over*. The model itself is stateless, meaning it remembers nothing on its own between requests. Everything it appears to "know" in the moment of deciding came from somewhere — your data, the vendor's corpus (their own collected pile of data), the open web, patterns it picked up during *training* (the one-time process where the model learned from huge amounts of text), the last few things that happened in the session. That assembled context is the entire basis for the decision. Nothing else is in the room.
...
**Where does the data come from?** Get specific. Is it deciding on *your* data, the vendor's proprietary dataset, the public web, patterns aggregated from other customers, or knowledge baked into a model during training two years ago and never updated since? Each has wildly different reliability and very different implications for whether the decision fits *your* reality.
...
**How trustworthy is the source?** Garbage in, confident garbage out. Is there provenance — a clear record of where each piece of information came from? Can the system distinguish an authoritative internal record from a scraped forum post? Does it weight them differently, or does everything in the context get treated as equally true?
...
**Can you see the context behind a specific decision?** This is the one that exposes most hand-wavy systems. For any given decision it made, can you reconstruct *what it saw* and *why it concluded what it did*? If the answer is "it's a black box, but it's very accurate" — a black box being a system you can only see the output of, never the workings inside — you have no way to catch the wrong ones, no way to improve it, and no defense when one blows up. Traceability — being able to trace any decision back to the exact information behind it — isn't a nice-to-have here. It's the whole basis for trust.
...
- "Our accuracy is 9X% on our benchmark." (A benchmark is just a standardized test the vendor scores their system against — so: on whose data, resembling whose reality, measured how? A number without provenance is marketing.)
...
- **Grounded and cited** — "grounding" means tying the system's answer to real, specific evidence instead of letting it free-associate; here, decisions point back to the exact sources behind them, so you can verify rather than trust.
